...
Make a histogram of the total number of steps taken each dayactivity$date <- as.Date(activity$date)
```{r}
library(dplyr)
total_steps_day <- activity %>% group_by(date) %>% summarize(sum(steps))
hist(total_steps_day$`sum(steps)`, col="red", main="Histogram of Daily Steps", xlab="Steps per day")
```
3) Mean and median number of steps taken each day
Calculate and report the mean and median of the total number of steps taken per day
```{r}
Avg_steps_day <- activity %>% group_by(date) %>% summarize(mean(steps))
Median_steps_day <- activity %>% group_by(date) %>% summarize(median(steps))
```
...
plot(Avg_steps_interval$interval, Avg_steps_interval$steps, type="l", ylab=" Average of Steps", xlab="Interval")
```
...
hist(total_steps_day_newdata$`sum(steps)`, col="blue", main="Histogram of Daily Steps with NAs", xlab="Steps per day")
```
